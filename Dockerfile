...
ENV ARCH='x86_64'
ENV RC_FILE='/home/${USERNAME}/.bashrc'
...
RUN echo 'alias vim=/usr/local/bin/nvim' >> ${RC_FILE}

USER ${USERNAME}

# nvm - Installation
RUN curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.40.3/install.sh | bash
RUN echo 'export NVM_DIR="$HOME/.nvm"' >> ${RC_FILE}
RUN echo '[ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh"' >> ${RC_FILE}
# Node - Installation
RUN /home/${USERNAME}/.nvm/nvm.sh install --lts

# Pyenv - Installation
RUN curl https://pyenv.run | bash
RUN echo "export PYENV_ROOT=/home/${USERNAME}/.pyenv" >> ${RC_FILE}
RUN echo '[[ -d $PYENV_ROOT/bin ]] && export PATH="$PYENV_ROOT/bin:$PATH"' >> ${RC_FILE} 
RUN echo 'eval "$(pyenv init - sh)"' >> ${RC_FILE}
RUN echo 'eval "$(pyenv virtualenv-init -)"' >> ${RC_FILE}
